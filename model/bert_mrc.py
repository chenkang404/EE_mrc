...
class BertQueryNER(nn.Module):
    def __init__(self, config):
        ...
        self.start_outputs = nn.Linear(config.hidden_size, 2)  #[768*2]
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, 
        ...
        start_extend = sequence_heatmap.unsqueeze(2).expand(-1, -1, seq_len, -1)  #unsqueeze()是进行维度扩充，给指定位置增加一个维数为1的维度。expand()返回tensor的一个新视图，单个维度扩大为更大的尺寸
        ...
        span_logits = torch.squeeze(span_logits)  # batch x seq_len x seq_len  #torch.squeeze()去掉维数为1的维度
