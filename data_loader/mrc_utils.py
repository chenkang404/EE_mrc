...
def convert_examples_to_features(examples, tokenizer, label_lst, max_seq_length, is_training=True, 
    allow_impossible=True, pad_sign=True,config = None):
    label_map = {tmp: idx for idx, tmp in enumerate(label_lst)}
    path_slot_descrip = os.path.join(config.data_dir,"slot_descrip")
    f = open(path_slot_descrip,"r",encoding="utf-8")
    qury_list =[qury.strip()  for qury in f.readlines()]

    query_dic = {id:qury for id ,qury in enumerate(qury_list)}
    features = []

    for (example_idx, example) in enumerate(examples):

        query_tokens = query_dic[int(example.query_item)]
        # whitespace_doc = whitespace_tokenize(example.context_item)
        whitespace_doc = example.context_item
        # print(list(whitespace_doc))
        ...
        if len(example.start_position) == 0 and len(example.end_position) == 0:
            ...
            for idx, (token, start_label, end_label) in enumerate(zip(whitespace_doc, fake_start_pos, fake_end_pos)):
                ...
                if len(tmp_subword_lst) > 1:
                    ...
                    print(id)
                    print("token", token)
                    print("tmp_subword_lst", tmp_subword_lst)
        ...
        for query_item in query_tokens:
            input_tokens.extend(tokenizer.tokenize(query_item))
            segment_ids.append(0) 
            start_pos.append(0)
            end_pos.append(0)
...
def read_mrc_event_examples(input_file, is_training=True, with_negative=True):
    """
    Desc:
        read MRC-NER data
    """
