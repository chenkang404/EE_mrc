...
from data_loader.mrc_data_processor import Conll03Processor, EventProcessor,MSRAProcessor, Onto4ZhProcessor, Onto5EngProcessor, GeniaProcessor, ACE2004Processor, ACE2005Processor, ResumeZhProcessor
...
def args_parser():
    ...
    parser.add_argument("--data_sign", type=str, default="event")
...
def load_data(config):
    ...
    elif config.data_sign =="event":
        data_processor = EventProcessor()
    else:
    ...
    VOCAB = 'vocab.txt'
    tokenizer = BertTokenizer4Tagger.from_pretrained(os.path.join(config.bert_model,VOCAB), do_lower_case=True)
...
def load_model(config, num_train_steps, label_list):
    ...
    n_gpu = config.n_gpu
    # n_gpu = torch.cuda.device_count() #自动检测gpu个数
...
def main():
    ...
    args_config.data_dir = "../data"
    args_config.bert_model = "../bert_base_chinese"
